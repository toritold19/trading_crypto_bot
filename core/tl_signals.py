...
def tl_strategy_signals(open_, high, low, close):
    ...
    if len(sz) < 3 or len(adx) < 3:
        return {
            ...
            "rsi": float("nan"), "rsi_ok": False, "buy_tl": False, "sell_tl": False
        }
    ...
    buy_tl = (cond_buy1 or cond_buy2) and rsi_ok

    cond_sell1 = is_ph_sz and adx_now < adx_prev
    cond_sell2 = is_ph_adx and sz_now < sz_prev and sz_now > 0
    sell_tl = (cond_sell1 or cond_sell2) and rsi_ok

    return {
        "sz": float(sz_now), "sz_prev": float(sz_prev),
        "adx": float(adx_now), "adx_prev": float(adx_prev),
        "is_pl_sz": bool(is_pl_sz), "is_ph_sz": bool(is_ph_sz),
        "is_ph_adx": bool(is_ph_adx), "rsi": float(rsi_now), "rsi_ok": bool(rsi_ok),
        "buy_tl": bool(buy_tl), "sell_tl": bool(sell_tl)
    }